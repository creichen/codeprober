...
| cpr_propertyListHide           | `String[]` or `List<String>` | null          | An list of property names to suppress in the list of properties when creating a new probe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | ast   |
...
| cpr_getOutput                  | any                          |               | Overrides how something is presented in the result of a probe evaluation. CodeProber supports extracting data from many standard datastructures by default, including arrays and anything that implements `java.util.Collection` or `java.lang.Iterable`. However, custom datastructures can use `cpr_getOutput` to define how to extract data from a given type. This logic is applied recursively: values returned by `cpr_getOutput` can also implement `cpr_getOutput` themselves, or be arrays/`java.util.Collection`'s, etc. A common way to implement `cpr_getOutput` is to return an object array, like `public Object[] cpr_getOutput() { return new Object[] { foo, bar }; }`.                                                                                     | any   |
