...
const doMain = (wsPort: number) => {
  ...
  function initEditor(editorType: string) {
	  ...
	  if (config.language) {
	      ...
	      settings.setSyntaxHighlighting(config.language);
	  }
	  ...
	  const overrideCfg = configureCheckboxWithHiddenButton(
	      uiElements.shouldOverrideMainArgsCheckbox, uiElements.configureMainArgsOverrideButton,
	  ...
	  const suffixCfg = configureCheckboxWithHiddenButton(
	      uiElements.shouldCustomizeFileSuffixCheckbox, uiElements.configureCustomFileSuffixButton,
	      (checked) => { // On checkbox update
